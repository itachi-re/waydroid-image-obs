...
Uses a more robust regex to find download links.
"""
...
def find_latest_file(directory, pattern_regex):
    ...
    # --- MODIFIED REGEX ---
    # Look for href attributes containing the project path and ending in .zip,
    # followed potentially by /download or other query parameters.
    # Capture the part ending in .zip
    link_pattern = r'href="(/projects/waydroid/files/images/[^"]+?\.zip)(?:/download|\?[^"]*)?"'
    all_links = re.findall(link_pattern, html)
    # --- END MODIFIED REGEX ---

    if not all_links:
        print(f"  No .zip links found matching pattern in directory: {directory}")
    ...
    for link_path in all_links:
        # link_path now should be like /projects/waydroid/files/images/.../file.zip
        ...
        if compiled_regex.match(filename):
            ...
            if date_match:
                ...
                # Construct the download URL by adding /download
    ...
    print(f"  Latest match: {latest_url.split('/')[-2]}") # Print filename
